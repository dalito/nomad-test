![docker image](https://github.com/dalito/nomad-test/actions/workflows/docker-publish.yml/badge.svg)


# dalito's NOMAD Oasis Distribution

This is the NOMAD Oasis distribution of dalito.
...
2. Clone the repository or download the repository as a zip file.
    ...
    ```sh
    git clone https://github.com/dalito/nomad-test.git
    cd nomad-test
    ```
    ...
    ```sh
    curl-L -o nomad-test.zip "https://github.com/dalito/nomad-test/archive/main.zip"
...
your `docker-compose.yaml` with `ghcr.io/dalito/nomad-test:main` for the services
...
and advanced configuration, see the [nomad-helm-charts](https://github.com/dalito/nomad-helm-charts) repository.
...
By default, no plugins are included in this distribution. You can find a list of available NOMAD plugins [here](https://nomad-lab.eu/prod/v1/oasis/gui/search/plugins). For a list of official plugins provided by FAIRmat, please see [here](https://github.com/dalito/.github/blob/main/profile/README.md). For inspiration, you can also check the list of [plugins that are installed on the production NOMAD deployment hosted by FAIRmat](https://gitlab.mpcdf.mpg.de/nomad-lab/nomad-distro/-/raw/main/pyproject.toml?ref_type=heads).
...
```sh
docker pull ghcr.io/dalito/nomad-test/jupyter:main
```
...
By default, documentation is built using the [nomad-docs](https://github.com/dalito/nomad-docs) repository. However, if you'd like to customize the documentation for your Oasis instance, you can easily do so.

1. First, [fork the nomad-docs repository](https://github.com/dalito/nomad-docs/fork).
...
_I get an_ `Error response from daemon: Head "https://ghcr.io/v2/dalito/nomad-test/manifests/main": unauthorized`
